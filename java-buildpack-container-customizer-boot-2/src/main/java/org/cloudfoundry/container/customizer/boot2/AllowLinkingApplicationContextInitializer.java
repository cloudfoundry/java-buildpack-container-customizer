...
public final class AllowLinkingApplicationContextInitializer implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    public void initialize(ConfigurableApplicationContext applicationContext) {
        if (areClassesPresent()) {
            applicationContext.getBeanFactory().registerSingleton(AllowLinkingWebServerFactoryCustomizer.class.getCanonicalName(), new AllowLinkingWebServerFactoryCustomizer());
        }
    }
    ...
    private boolean areClassesPresent() {
        ...
        return isPresent("org.apache.catalina.Context", classLoader)
            && isPresent("org.springframework.boot.web.embedded.tomcat.TomcatServletWebServerFactory", classLoader);
    }
